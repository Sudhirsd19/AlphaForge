...
def calculate_cost(cost_input: CostInput) -> CostResult:
    ...
    # Reuse the authoritative Phase 5 notional calculator; Phase 6 does not redefine it.
    entry_notional = calculate_notional(effective_entry, quantity, multiplier)
    exit_notional = calculate_notional(effective_exit, quantity, multiplier)
    ...
    entry_slippage_cost = (
        abs(effective_entry - cost_input.entry_reference_price)
        * Decimal(quantity)
        * multiplier
    )
    exit_slippage_cost = (
        abs(effective_exit - cost_input.exit_reference_price)
        * Decimal(quantity)
        * multiplier
    )
    ...
    if cost_input.side is TradeSide.LONG:
        gross_pnl = (effective_exit - effective_entry) * Decimal(quantity) * multiplier
    else:
        gross_pnl = (effective_entry - effective_exit) * Decimal(quantity) * multiplier
