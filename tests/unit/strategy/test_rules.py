...
def test_evaluate_trend_regime_insufficient_bars() -> None:
    candles = [
        create_candle(datetime(2026, 9, 12, 10, i, tzinfo=UTC), "100", "110", "90", "105")
        for i in range(10)
    ]
...
def test_evaluate_breakout_inside_range() -> None:
    candles = [
        create_candle(datetime(2026, 9, 12, 10, i, tzinfo=UTC), "100", "110", "90", "100")
        for i in range(25)
    ]
...
def test_evaluate_candle_geometry_doji_rejected() -> None:
    ...
    doji = create_candle(
        datetime(2026, 9, 12, 10, 0, tzinfo=UTC), "100.00", "110.00", "90.00", "100.50"
    )
    is_valid, body_ratio, _ = evaluate_candle_geometry(
